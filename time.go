// Persian Time tools
// Please visit https://github.com/ghoroubi/go-ptime for more information.
// Copyright (c) 2019 Nima Ghoroubi
package persiantime
...
// Month represents the month of the year starting from Farvardin = 1.
type Month int

// Weekday represents the day of the week starting from Shanbe = 0.
type Weekday int

// AmPm represents the 12-Hour marker.
type AmPm int

// Time represents the persian time.
type Time struct {
	year  int
	month Month
	day   int
...
// Constants of AM-PM markers.
const (
	Am AmPm = 0 + iota
	Pm
)

var longAMPM = [2]string{
	"قبل از ظهر",
	"بعد از ظهر",
}

var shortAMPM = [2]string{
	"ق.ظ",
	"ب.ظ",
}
...
// count of days per each month.
var dayOfMonthCount = [12][3]int{
	{31, 31, 0},   // Farvardin
...
// Iran represents the Asia/Tehran time.Location.
...
func (t Time) YearDay() int {
	return dayOfMonthCount[t.month-1][2] + t.day
}

// RYearDay returns the number of remaining longDaysName of the year of t.
...
// RMonthDay returns the number of remaining longDaysName of the month of t.
func (t Time) RMonthDay() int {
	i := 0
	if t.IsLeap() {
		i = 1
	}
	return dayOfMonthCount[t.month-1][i] - t.day
}
...
func (t Time) LastMonthDay() Time {
	...
	ld := dayOfMonthCount[t.month-1][i]
	if ld == t.day {
		return t
	}
...
// AddDate returns a new instance of Time for t.year+years, t.month+months and t.day+longDaysName.
...
//		W                week of month
//		RD               remaining longDaysName of year
//		D                day of year
//		rd               remaining longDaysName of month
...
func (t *Time) normDay() {
	...
	between(&t.day, 1, dayOfMonthCount[t.month-1][i])
}
